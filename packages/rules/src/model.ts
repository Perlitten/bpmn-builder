...
function emptyModel(parseError?: string): LintModel {
  return { nodes: [], flows: [], associations: [], adHocInnerIds: [], bounds: new Map(), labels: new Map(), hasDi: false, ...(parseError ? { parseError } : {}) };
}
...
function parseBoundsAndLabels(xml: string): { bounds: Map<string, Bounds>; labels: Map<string, Bounds> } {
...
function fromXml(xml: string): LintModel {
  ...
  return { nodes, flows, associations, adHocInnerIds, bounds, labels, hasDi: bounds.size > 0 };
}
