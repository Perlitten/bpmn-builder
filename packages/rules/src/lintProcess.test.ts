...
describe('lintProcess', () => {
  ...
  it('keeps unnamed parallel splits and branches valid', () => {
    let process = createProcess();
    process = addTask(process, { name: 'Review request' }).process;
    process = splitParallel(process, {
      after: process.nodes.find((node) => node.name === 'Review request')!.id,
    }).process;

    const result = lintProcess(process);
    expect(
      result.warnings.filter(
        (finding) => finding.id === 'quality.unnamed-gateway' || finding.id === 'quality.unlabeled-branch',
      ),
    ).toEqual([]);
  });
