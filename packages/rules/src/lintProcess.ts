...
function layerGeometry(model: LintModel, suggestions: Finding[]): LayoutSource {
  ...
  const labelEntries = Array.from(model.labels.entries());
  const nodeEntries = Array.from(model.bounds.entries());
...
function matchesLayoutEngine(model: LintModel): boolean {
  ...
  return ids.every((id) => sameBox(model.bounds.get(id), expected[id]));
}
